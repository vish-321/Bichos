...
from JAMButton import JAMLabel

class JAMElipseButton(pygame.sprite.Sprite):
	def __init__(self,
		...
		# Si solo hay una imagen, sin tamaño y el tamaño del panel es el default, el boton tendrá el tamaño de la imagen
			...
			# si no pasas tamaño a la imagen
				...
				# si pasas tamaño al panel
					jambasebuton=JAMElipseBaseButton(color_relleno=self.color_relleno,
	...
	# genera los componentes internos del botón
		...
		self.Label = JAMLabel(imagen=self.imagen,
			...
			color=self.color)
		# Base del Botón
		self.Base = JAMElipseBaseButton(color_relleno=self.color_relleno,
			color_borde_relleno=self.color_borde_relleno,
			color_panel=self.color_panel,
			color_borde_panel=self.color_borde_panel,
			tamanio_panel=self.tamanio_panel,
...
# -------------------- FIN DE la clase JAMElipseButton -------------------- #

# -------------------- INICIO DE la clase JAMElipseBaseButton -------------------- #
class JAMElipseBaseButton(pygame.sprite.Sprite):
# el texto del botón
	...
	# -------------------- INICIO DE METODOS DE CONSTRUCCION DE JAMElipseBaseButton -------------------- #
	...
	# genera una superficie
		...
		pygame.draw.ellipse(superficie, color_relleno, rectangulo_borde, 0)
		if grosor_borde and color_borde: pygame.draw.ellipse(superficie, color_borde, rectangulo_borde, grosor_borde)
		return superficie
	# -------------------- FIN DE METODOS DE CONSTRUCCION DE JAMElipseBaseButton -------------------- #
	...
	# -------------------- FIN DE METODOS INTERNOS AUTOMÁTICOS -------------------- #
# -------------------- FIN DE la clase JAMElipseBaseButton -------------------- #
